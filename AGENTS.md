...
`dist/` is committed because Pi installs Git packages with production dependencies only; rebuild and commit it after source changes. `legacy.ts` preserves the original Remendra entry for explicit rollback. The architecture and configuration descriptions below refer to that legacy engine unless they name `src/v2`. Shared workflow and testing rules still apply. V2 documentation lives in README.md and docs/V2.md.
...
```bash
pnpm test          # vitest run (all tests, 83 files, no network)
...
- CI order: `build` → `typecheck` → `lint` → `test` → `test:smoke` → `format:check` (.github/workflows/ci.yml).
...
- `src/core/` — unified config (`unified-config.ts` = defaults + resolution; env overrides declared in `config-env.ts` as `PI_REMENDRA_*`). configManager is the true source and entry point - users edit in UI.
...
- `src/commands/` — `pi-vcc.ts` (`/remendra`), `memory.ts` (`/remendra-memory`), `vcc-recall.ts` (`/remendra-recall`), `remendra-export.ts` (`/remendra-export`), `cleanup.ts`.
...
- Runtime clone for local testing: `~/.pi/agent/git/github.com/yoda-digital/pi-remendra/` — sync changes there and `/reload` Pi.
- `debug: true` → pre-compaction snapshot at `/tmp/pi-remendra-debug.json`; `debugLog: true` → JSONL at `~/.pi/agent/pi-remendra/debug.ndjson`.
- Config lives at `~/.pi/agent/pi-remendra/pi-remendra-config.json`; cooldowns at `pi-remendra-cooldown.json`. `PI_REMENDRA_PASSIVE=true` disables compaction + memory entirely.
